...
exports.createNewUser = async (req, res) => {
    try {
        ...
        // email verify OTP
        mailer(`${firstname} ${lastname}`, email, "Email Verification", `<div class="lead">
...
exports.loginUser = async (req, res) => {
    ...
    try {
        ...
        const decryptPassword = await bcrypt.compare(password, checkIfEmailExists.password);
        if (!decryptPassword) {
            return res?.json({
                status: false,
                message: "invalid credentials",
...
exports.ResendOTP = async (req, res) => {
    try {
        ...
        //logic to send otp to email
        mailer(`${checkIfEmailExists.firstname}, ${checkIfEmailExists.lastname}`, email, `OTP RESEND`, `<div class="lead">
        Use the verification code below to complete your action. This code is valid for <strong>5 minutes</strong>.
      </div>
      <div class="otp-box">
        <div class="otp-code">${generateOtpToken}</div>
...
exports.verifyOtp = async (req, res) => {
    try {
        ...
        if (otpType == "verifyEmail") {
            ...
            mailer(`${checkIfEmailExists.firstname}, ${checkIfEmailExists.lastname}`, email, `Verification Successful 🎉`, ` <div class="content">
      ...
      You can now enjoy full access to Cardinal Publishing Ink and explore thousands of amazing books at your fingertips.</p>
        ...
        if (otpType == "passwordReset") {
            const generatePasswordResetToken = jwt.sign({ id: checkIfEmailExists.id, email: email, tokenType: "passwordReset" }, process.env.JWT_SECRET_KEY, { expiresIn: "1h" });
            return res?.json({
                status: true,
...
exports.forgottonPassword = async (req, res) => {
    try {
        ...
        //email logic
        mailer(`${checkIfEmailExists.firstname}, ${checkIfEmailExists.lastname}`, email, "Password Reset OTP", `<div class="lead">
        Use the verification code below to complete your action. This code is valid for <strong>5 minutes</strong>.
      </div>
      <div class="otp-box">
        <div class="otp-code">${generateOtpToken}</div>
...
exports.resetPassword = async (req, res) => {
    try {
        ...
        mailer(`${checkIfEmailExists.firstname}, ${checkIfEmailExists.lastname}`, checkIfEmailExists.email, `Password Reset Successful`, ` <div class="content">
